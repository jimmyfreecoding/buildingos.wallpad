...
const resetLight = (l) => {
  ...
  const infoWinIconUrl = lightStatus === 'on'
    ...
    : '/wallpad/static/images/lightoff.png'
  const fontColor = lightStatus === 'on' ? '#000' : '#919495'
  const name = l?.name?.replace(`${obj.value?.code}区`, '')

  const content = `
    <div style="padding:6px;background:${bgColor};border-radius:10px;color:${fontColor};font-size:30px;position:relative!important;z-index:1!important;display:flex;justify-content:center;align-items:center;" >
  ...
  const infoWindow = new AirocovMap.covers.InfoWindow(map.value, {
    content,
    position: { x: l.posX, y: l.posY, z: l.posZ },
  })
...
const show = (e) => {
  ...
  if(localStorage.getItem("data")){
    ...
    area = jsondata.spaceObj[jsondata.spaceIndex].floorArea[jsondata.floorareaIndex].floor[jsondata.floorIndex].area[jsondata.areaIndex]
    x = parseFloat(area.posX).toFixed(2)
    y = 0.06
    z = parseFloat(area.posZ).toFixed(2)
  }
  console.log("area",x)
  console.log("area",y)
  console.log("area",z)
  ...
  if(map.value==null|| document.getElementById("mapId").innerHTML==''){
    ...
    map.value = new AirocovMap.Map({
      ...
      onReady: function () {
        ...
        // 首次打开照明
        console.log("map init!!!!!!!!!!!!!",tabIndex.value)
        if (tabIndex.value === 2) {
          setLight()
          show(obj.value?.code)
        }

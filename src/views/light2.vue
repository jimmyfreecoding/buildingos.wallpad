<template>
  ...
  <div class="lights-box">
    <div class="flex-row justify-around" style="margin-top: 22px">
      <div>
        <!-- <IndexTitle title="照明控制"></IndexTitle> -->
        <div class="content">
          <div class="title">照明设置</div>
          <sliderPick
            size="mini"
            style="width: 100%; background: #2e3a4d; margin-bottom: 16px"
          ...
          <div class="title_o" v-if="dlightshow">亮度调节</div>
          <div class="wendu-box liangdu" v-if="dlightshow">
            <div class="click-box flex-row justify-between"></div>
            <!-- 拖动的小圆点pc样式和平板样式不对应，目前css样式pad显示正常 -->
            <el-slider
              v-model="acObject.ajust"
              :min="1"
              :max="255"
              :step="1"
              show-tooltip
              ...
              :marks="dlightMarks"
              @change="handleChangeDlight"
            />
            <div class="min-temp-control">
              <span class="floor">昏暗</span>
              <span class="celling">明亮</span>
            </div>
          </div>
        </div>
        <div style="display: none; justify-content: space-between">
          <div class="content2">
            <div class="title">空气净化器</div>
            <div class="btn">
              <div>全关</div>
              <div class="btn-active">全开</div>
            </div>
          </div>
          <div class="content2">
            <div class="title">排风系统</div>
            <div class="btn">
              <div>全关</div>
              <div class="btn-active">全开</div>
            </div>
      ...
      <div>
        ...
        <div class="content">
          <div class="title">空调设置</div>
          <div v-if="acloadingFlag" style="font-size: 32px">
            空调指令执行中....
          </div>
          <sliderPick
            size="mini"
            style="width: 100%; background: #2e3a4d; margin-bottom: 22px"
            v-if="!acloadingFlag"
        ...
        <div class="content" v-if="blindShow">
          <div class="title">窗帘控制</div>
          <div class="curtain">
            <div @click="handleBlind('up')">
              <img src="@/assets/images/curtainopen.png" alt="" />
              打开
            </div>
            <div @click="handleBlind('pause')">
              <img src="@/assets/images/curtainpause.png" alt="" />
              暂停
            </div>
            <div @click="handleBlind('down')">
              <img src="@/assets/images/curtainclose.png" alt="" />
              关闭
            </div>
          </div>
        </div>
      ...
      <div>
        ...
        <div class="left-box-box">
          <div class="content" style="height: calc(100vh - 104px)">
            <div class="title">室内环境</div>
            <div
              class="align-center flex-row justify-between"
              style="flex-direction: column; height: 580px"
            >
              ...
              <div class="main qita flex-col justify-around">
                <div class="justify-between">
                  ...
                  <div class="tem-box">
                    <div>
                      {{ airValue(inside.tvoc, "tvoc").showValue }}mg/m³
                    </div>
                  </div>
                </div>
                <el-progress
...
:deep(.el-slider__marks-stop) {
  background-color: var(--el-slider-stop-bg-color) !important;
}
...
.liangdu {
  .el-slider__runway {
    background: linear-gradient(90deg, #c1c1c1 0%, #f0c425 92.5%) !important;
  }
  .el-slider__button-wrapper {
    &::after {
      background-color: #dec361 !important;
    }
  }
}
.wendu-box {
...
const dlightshow = ref(false);
const blindShow = ref(false);
...
const handleChangeDlight = (val) => {
  publish(
    `/iot/action/dlight/HGH-WC/${obj.value.floorarea}/${obj.value.floor}/`,
    JSON.stringify({ ajust: val, action: "on" })
  );
  publish(
    `/iot/action/dlight/HGH-WC/${obj.value.floorarea}/${obj.value.floor}/`,
    JSON.stringify({ action: "status" })
...
const handleBlind = (action) => {
  publish(
    `/iot/action/blind/HGH-WC/${obj.value.floorarea}/${obj.value.floor}/`,
...
getMessage((topic, message) => {
  ...
  if (
    ...
    if (devices?.dlight && devices.dlight?.length) {
      dlightshow.value = true;
      const arr = devices.dlight?.map((x) => {
        return { ...x, status: JSON.parse(x.status) };
      });
      const ajust = majorityCandidate(arr, "ajust");
      acObject.value.ajust = ajust;
    }
    if (devices.blind && devices.blind?.length) blindShow.value = true;
